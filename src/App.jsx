...
const ALL_PROJECTS = [
    { id: '01', title: 'CryptoVol Dashboard', category: 'Personal', domain: 'Dev', date: '2026.03 —', description: 'GARCH 적합 연산(수백ms) 지연 → 5분 TTL 캐싱 + 에러 격리로 6개 모형 안정 서빙(14 REST + 1 WS). WebSocket 릴레이로 API 키 노출 차단, 1개 모형 실패가 전체 장애로 번지지 않는 운영 구조.', overview: '2023년 논문이 기간에 쫓겨 남긴 미구현 모형과 미검증 수치를, 3년 뒤 단독으로 구현·재계산한 개인 풀스택 프로젝트. 캐싱, Rate Limiting, WebSocket 릴레이, pytest 32개 + GitHub Actions로 운영 가능한 모델 서빙 흐름을 설계하고 Render+Vercel+Neon으로 배포.', role: 'AI 모델 API 서빙, WebSocket 릴레이, 캐싱/Rate Limiting, Docker 기반 배포 구조, 에러 격리 설계', tech: ['FastAPI', 'Docker', 'GitHub Actions', 'WebSocket', 'Render/Vercel'], github: 'https://github.com/ykgstar37-lab/crypto-volatility-intelligence-system', link: '/projects/crypto-vol-dashboard', thumbnail: thumbCryptoVol, badge: 'Personal', demoHash: 'screenshots' },
    { id: '02', title: 'Seoul Culture Map', category: 'Personal', domain: 'Dev', date: '2026.03 — 2026.04', description: '정적 분석 결과를 11개 API 엔드포인트로 서빙. 2,500+ 시설 데이터를 K-means 군집분석 + Leaflet 지도로 시각화하고, Intent 라우팅으로 AI 호출 비용을 85% 절감.', overview: '서울시 2,500+ 문화시설 데이터를 고객 요구 조건(지역/목적/접근성)에 맞춰 검색·추천하는 데이터 서비스. SQL/벡터 검색/SSE를 조합해 비용과 응답 품질을 제어.', role: 'RESTful API 설계(11개 엔드포인트), Intent 라우팅, SQL+Vector 검색 연계, SSE 스트리밍, 비용 최적화', tech: ['FastAPI', 'React', 'SQLite', 'Vector DB', 'SSE'], github: 'https://github.com/ykgstar37-lab/seoul-culture-map', link: '/projects/seoul-culture-map', thumbnail: thumbSeoulCulture, badge: 'Personal', demoHash: 'screenshots' },
    ...
    { id: '05', title: 'SubFlow — Subscription Manager', category: 'Personal', domain: 'Dev', date: '2026.03 — 2026.09', description: '흩어진 구독 지출 파악 문제 → 자동 수집이 불가능함을 사전 조사로 확인하고 카탈로그 기반 Web/Mobile/FastAPI 공유 구조로 전환. 11개 DB 테이블, 8종 API 라우터, 99종(15카테고리)·요금제 198개 서비스 카탈로그로 지출 분석·중복 감지·환율/부가세 환산·알림 실발송·뉴스 AI 요약까지 통합. 웹앱·API 배포 완료, iOS App Store 출시.', overview: 'React Web과 Expo Mobile이 단일 FastAPI 백엔드를 공유하는 구독 관리 플랫폼. 총액, 다음 결제일, 예산 초과, 중복 구독처럼 반복 사용자가 바로 행동할 지표를 우선 설계.', role: '기획·설계·웹/모바일 프론트·FastAPI 백엔드·배포를 풀스택 단독 담당, PostgreSQL 11테이블 스키마, JWT 인증, 지출 분석/알림 도메인', tech: ['FastAPI', 'React', 'React Native', 'PostgreSQL', 'Zustand'], github: 'https://github.com/ykgstar37-lab/SubFlow', link: '/projects/subflow', thumbnail: thumbSubFlow, badge: 'Personal', demoHash: 'demo' },
    { id: '06', title: '암호화폐 변동성 비교 및 분석: GARCH 모델 기반 예측', category: 'Team', domain: 'Data', date: '2023.12 — 2024.01', description: 'ADF 단위근 검정으로 로그수익률 변환 근거를, ARCH-LM 검정으로 GARCH 적용 근거를 확보. GARCH→TGARCH→외생변수 순으로 모형을 확장해 비대칭 레버리지 효과를 확인하고, 거래량·FNG가 설명력을 높인다는 결론을 도출.', overview: '암호화폐 변동성을 GARCH 계열 모형으로 비교 분석한 3인 팀 연구 프로젝트. 기간 제약으로 GARCH·TGARCH·TGARCH+E.V까지 적합했고, HAR 계열은 수식 서술에 그쳤다.', role: 'GARCH 모형 비교 분석, ADF·ARCH-LM 사전 검정, 외생변수 효과 분석, 발표', tech: ['Python', 'GARCH', 'HAR'], github: 'https://github.com/ykgstar37-lab/crypto-volatility-intelligence-system', link: '/projects/crypto-volatility', thumbnail: thumbCrypto, badge: 'Team', demoHash: 'presentation' },
...
const PROBLEM_SOLVING_CASES = [
    {
        ...
        problem: '카탈로그 요금제 198개 중 97개(49%)가 외화(USD) 기준이고 결제 주기도 주·월·분기·연이 섞여 있다. 총액·카테고리 비중·예산 초과·기간별 추이가 각자 환산하면 화면마다 다른 숫자가 나오는데, 예외를 던지지 않고 조용히 어긋나 발견이 어렵다. 해외 서비스는 표시가에 부가세 10%까지 붙는다.',
        solutions: [
        ...
        results: [
            ...
            { label: '외화 노출 요금제', value: '97 / 198 (49%)' },
