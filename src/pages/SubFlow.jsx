...
const chart = String.raw`flowchart LR
  ...
  Auth --> API["FastAPI /api/v1<br/>8 routers"]
  API --> Domain["Service Layer<br/>Subscription / Analytics / Renewal<br/>Notification / Digest / News"]
...
const stats = [['97 / 198','USD 요금제','요금제의 절반이 환율에 노출'], ['11','DB Tables','normalized schema'], ['8','Routers','/api/v1 core APIs'], ['116','Backend Tests','14개 파일 · pytest']];
...
const narratives = [
  ['외부 구독 연동의 한계 → 카탈로그 기반 전환',
   ...
   '서비스 99종·요금제 198개·카테고리 15개 카탈로그 구축 → 검색·선택만으로 등록. 이 카탈로그가 이후 요금 인상 감지·중복 구독 판정의 전제 조건이 됨.'],
  ...
  ['"요금이 올랐다"를 어떻게 아는가 → 카탈로그를 구독과 분리',
   ...
   '카탈로그(services / service_plans)를 사용자 구독(subscriptions)과 분리하고, plan_price_history가 요금제별 가격 변동을 기록하도록 설계. 15개 카테고리는 단순 분류가 아니라 중복 구독 감지의 판정 기준으로 사용.',
  ...
  ['금액 기준 통일 → 취향이 아니라 데이터가 강제한 요구사항',
   '요금제 198개 중 97개가 외화(USD) 기준이다. 카탈로그의 절반이 환율에 노출돼 있고 해외 서비스는 표시가에 부가세 10%까지 붙어, 주기·통화·세금을 각자 환산하면 총액과 비중이 조용히 어긋난다.',
   'utils/cost.py에 "월 단위 KRW" 단일 환산 함수를 두고 총액·비중·예산·분석이 전부 이 진입점을 거치게 하고, 부가세 계산은 utils/vat.py로 분리. 전 구간 Decimal로 처리해 부동소수점 오차를 배제.',
...
const contributions = [
  ...
  ['프로덕션 배포와 출시 준비까지 완주','기능 구현에서 끝내면 실제 사용자가 쓸 수 없는 문제 -> Railway(백엔드+관리형 Postgres)·Cloudflare Pages(웹앱·랜딩·약관)·EAS(iOS/Android 빌드) 배포와 Resend 메일 파이프라인(DKIM/SPF) 구성 -> api / app / 랜딩을 각각 커스텀 도메인+SSL로 분리 배포하고, iOS는 EAS Submit으로 App Store 출시까지 완료.'],
];

// 핵심 기능 (PORTFOLIO.md · 데모 영상 순서 기준)
const demoFeatures = [
  ['01','계정 · 인증','이메일 회원가입 → 인증 메일(Resend) → JWT 로그인. 비밀번호 재설정과 계정 삭제까지 갖췄고, 인증 엔드포인트는 분당 5~10회로 제한하며 없는 이메일에도 같은 응답을 돌려 가입자 명단이 드러나지 않게 처리.'],
  ['02','구독 등록','카탈로그 99종·요금제 198개에서 검색·선택으로 추가하고, 없는 요금제는 직접 입력. 표시가에 붙는 부가세 10%를 반영해 실제 결제액을 먼저 보여주며 주·월·분기·연 주기와 활성·일시정지·해지·체험 상태를 지원. 서비스마다 cancel_url을 저장해 해지 페이지로 바로 이동.'],
  ['03','대시보드','월/연 총 지출·활성 구독 수·다음 결제일·구독 건강 점수를 한 화면에 요약. 7일 내 결제 예정 위젯으로 임박 결제 강조.'],
  ['04','지출 분석','카테고리별 지출 비중 도넛 + 3/6/12/24개월 추이 영역 차트. 다운그레이드·해지·주기 전환 절약 인사이트를 원클릭 적용.'],
  ['05','캘린더 · 타임라인','반복 결제를 달력에 자동 전개하고, 구독 생성·플랜 변경·해지 이력을 타임라인으로 추적.'],
  ['06','예산 · 알림','월 예산은 이번 달에 실제로 빠져나가는 금액으로 판정한다(연간 구독을 12로 나누지 않음). 결제 N일 전 이메일·푸시 실발송, 요금 인상·중복·환율 알림을 인박스로 통합 관리.'],
  ['07','고급 인사이트','같은 카테고리 중복 구독 감지, 외화 구독 환율 추적(Frankfurter 기준일 표기), 가족·팀 공유 구독의 1인당 분담액, 요금 인상 이력(plan_price_history) 관리.'],
  ['08','뉴스 + AI 요약','내 구독 서비스 기준으로 뉴스 개인화 정렬, 기사 탭 시 OpenAI(gpt-4o-mini) AI 요약 모달 제공.'],
];
...
const challenges = [
  ...
  ['외화 환율의 최신성 vs 호출 비용','환율은 계속 바뀌는데 매 요청마다 외부 API를 호출하면 지연과 장애 전파가 그대로 사용자에게 간다','스케줄러 잡으로 미리 당겨오는 대신 exchange_rate.py에서 요청 시점 1시간 TTL 캐시로 조회하고, 응답의 기준일(date)을 보관해 UI가 "언제 기준 환율인지" 밝히게 하고, API 장애 시 폴백 환율로 대체','외부 호출을 통화당 시간당 1회로 줄이면서, 환율 API가 죽어도 화면이 유지되고 표시값이 실시간인 척하지 않는 구조'],
...
function DrawerTabs(){return [{label:'About',content:<div className="space-y-4"><p className="rounded-2xl p-6 text-sm leading-relaxed text-white" style={{background:'linear-gradient(135deg,#14b8a6,#2563eb)',wordBreak:'keep-all'}}>흩어진 구독 지출을 Web/Mobile에서 한 번에 관리하는 플랫폼. 결제 내역 자동 수집이 현실적으로 불가능함을 사전 조사로 확인하고 카탈로그 선택 방식으로 전환해, 총액·결제일·예산 초과·중복·환율 같은 관리형 인사이트에 집중했습니다. 기획·설계·웹/모바일·백엔드·배포를 풀스택 단독으로 담당.</p><div className="grid grid-cols-2 sm:grid-cols-4 gap-3">{stats.map(([v,l,s])=><div key={l} className="bg-gray-50 p-4 rounded-xl text-center"><p className="text-xl font-black" style={{color:PRIMARY}}>{v}</p><p className="text-[10px] font-bold uppercase text-gray-400">{l}</p><p className="text-[10px] text-gray-500">{s}</p></div>)}</div></div>},{label:'Key Features',content:<div className="grid grid-cols-1 sm:grid-cols-2 gap-3">{[['구독 관리','서비스 카탈로그에서 추가 + 커스텀 직접 입력'],['대시보드','월/연 총 지출·활성 구독 수·다음 결제일 요약'],['지출 분석','카테고리별 비중 + 월별 추이 차트'],['캘린더','과거/미래 결제일을 한눈에'],['변경 타임라인','생성·플랜 변경·해지 이력 추적'],['예산 관리','월 예산 설정 및 초과 알림'],['환율 추적','외화 구독 KRW 자동 변환 + 변동 알림'],['중복 감지','같은 카테고리 내 겹치는 서비스 탐지'],['절약 제안','불필요·중복 구독 기반 비용 절감 추천'],['알림 설정','결제 N일 전·이메일/푸시 실제 발송'],['뉴스 + AI 요약','구독 서비스 관련 뉴스 + OpenAI 기사 요약'],['가족·공유 분담','member_count 설정 시 1인당 몫으로 지출 재계산'],['CSV 내보내기','UTF-8 BOM으로 Excel에서 바로 열리는 CSV 내보내기'],['부가세 반영','해외 서비스 표시가에 붙는 부가세 10%를 실부담액에 반영'],['오류 신고','앱 안에서 사진 첨부와 함께 오류를 제보 (/feedback/contact)'],['다크모드 · 한/영','다크모드와 다국어 토글 — 번역이 없으면 한국어 원문으로 폴백'],['온보딩','첫 가입 시 4단계 온보딩으로 초기 설정 안내']].map(([t,d])=><div key={t} className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm"><p className="text-sm font-bold text-gray-900 mb-1">{t}</p><p className="text-xs text-gray-500 leading-relaxed" style={{wordBreak:'keep-all'}}>{d}</p></div>)}</div>},{label:'Tech Stack',content:<div className="flex flex-wrap gap-2">{chips.concat(['SQLAlchemy 2.0(async)','Alembic','Pydantic','Docker','slowapi','APScheduler','OpenAI','Frankfurter API']).map(x=><span key={x} className="px-4 py-2 bg-white text-gray-700 text-sm font-medium rounded-full border border-gray-200 shadow-sm">{x}</span>)}</div>},{label:'API',content:<div className="space-y-3">{['Auth: POST /auth/register · /login · /refresh · /verify-email · /reset-password','Services: GET /services · /services/popular · /services/{id}/price-history','Subscriptions: GET·POST /subscriptions · /from-catalog · /upcoming · /calendar-events · /timeline · /export','Analytics: GET /analytics/overview · /spending-trend · /category-breakdown · /overlaps · /savings-suggestions · /budget-status · /price-changes','Categories: GET /categories','Notifications: GET·PUT /notifications/settings · /inbox · PUT /push-token · POST /deliver · /digest','News: GET /news, POST /news/summary (AI 요약)','Feedback: POST /feedback/contact (오류 신고 · 사진 첨부)'].map(x=><div key={x} className="rounded-xl border border-gray-100 bg-white p-4 text-sm text-gray-600"><span className="font-bold" style={{color:PRIMARY}}>{x.split(':')[0]}</span>{':'+x.split(':').slice(1).join(':')}</div>)}</div>},{label:'DB Schema',content:<div className="grid grid-cols-1 sm:grid-cols-2 gap-3">{['users','categories','services','service_plans','plan_price_history','subscriptions','subscription_history','payment_history','notification_settings','notifications','news_cache'].map(x=><div key={x} className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm"><p className="font-mono text-sm font-bold" style={{color:SECONDARY}}>{x}</p></div>)}</div>},{label:'Testing',content:<div className="space-y-3"><p className="text-sm text-gray-600 leading-relaxed" style={{wordBreak:'keep-all'}}>pytest 14개 파일 116개 테스트로 인증·구독 CRUD·분석 집계·카탈로그 소유권·월 단위 KRW 환산·부가세·자동 갱신·알림 설정을 커버합니다. 보안 하드닝(응답 통일·rate limit) 이후에는 변경된 보안 동작에 맞춰 테스트를 갱신했고, rate limiter는 테스트 환경에서 비활성화해 테스트 간 상태 오염을 막았습니다.</p><div className="grid grid-cols-2 sm:grid-cols-3 gap-2">{['test_auth','test_subscriptions','test_analytics','test_cost','test_vat','test_renewal','test_renewal_notifications','test_notifications','test_push_settings','test_catalog_ownership','test_custom_plans','test_services','test_categories','test_feedback'].map(x=><div key={x} className="rounded-lg border border-gray-100 bg-white p-3 text-center"><p className="font-mono text-xs font-bold" style={{color:PRIMARY}}>{x}</p></div>)}</div></div>}];}
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
      ...
      <motion.div initial="hidden" animate="visible" variants={fadeInUp} className="mb-20">
        <div className="flex items-center gap-3 mb-4 flex-wrap">
          ...
          <span className="text-[10px] font-bold px-3 py-1 bg-emerald-50 text-emerald-700 rounded-full tracking-wider uppercase">Web 배포 완료 · iOS App Store 출시</span>
        </div>
        ...
        <p className="text-lg text-gray-500 font-medium leading-relaxed mb-6" style={{wordBreak:'keep-all'}}>결제 내역 자동 수집이 현실적으로 불가능함을 사전 조사로 확인하고, 방향을 바꿔 서비스 카탈로그(99종·요금제 198개) 기반으로 재설계했습니다. React Web과 Expo Mobile이 단일 FastAPI 백엔드를 공유하도록 API-first로 설계해, 11개 DB 테이블·8개 라우터 위에서 구독 등록·지출 분석·중복 감지·환율/부가세 환산·개인화 알림 실발송·뉴스 AI 요약을 하나의 흐름으로 묶었습니다.</p>
        <div className="flex gap-3 flex-wrap">
          <a href="https://github.com/ykgstar37-lab/SubFlow" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 bg-gray-900 hover:bg-gray-700 text-white text-sm font-medium rounded-full transition">
          ...
          <a href="https://app.mysubflow.app" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 text-white text-sm font-medium rounded-full transition hover:brightness-110" style={{backgroundColor:SECONDARY}}>
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 12a9 9 0 11-18 0 9 9 0 0118 0zM3.6 9h16.8M3.6 15h16.8M12 3a15 15 0 010 18 15 15 0 010-18z" /></svg>
            Web App
          </a>
          <a href="https://api.mysubflow.app/docs" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 hover:border-gray-400 text-gray-700 text-sm font-medium rounded-full transition">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="p-6 sm:p-8 space-y-6">
            ...
            <div>
              ...
              <p className="text-gray-600 leading-relaxed" style={{wordBreak:'keep-all'}}>기획·설계부터 React 웹 대시보드, Expo 모바일, FastAPI 백엔드(11개 테이블·8개 라우터), 배포까지 풀스택을 단독으로 담당했습니다. 실현 가능성 조사, 점진적 스키마 마이그레이션, API-first 설계, 개인화 알림 파이프라인, 프로덕션 기준 보안 하드닝을 직접 결정하고 구현했습니다.</p>
            </div>
            <div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white p-6 sm:p-8 rounded-2xl border border-gray-100 shadow-sm">
            ...
            <ul className="space-y-2 text-sm text-gray-600">
              ...
              <li className="flex items-start gap-2"><span className="text-amber-500 text-xs font-bold mt-0.5 w-14 shrink-0">진행중</span><span>출시 — 웹앱·API·랜딩은 커스텀 도메인으로 배포 완료, iOS는 App Store 출시 완료(2026.09). Android(Play Console)와 소셜 로그인(Google/Kakao)은 다음 단계</span></li>
