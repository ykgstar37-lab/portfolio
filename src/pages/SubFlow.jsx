...
const stats = [['96 / 196','USD 요금제','요금제의 절반이 환율에 노출'], ['11','DB Tables','normalized schema'], ['8','Routers','/api/v1 core APIs'], ['116','Backend Tests','14개 파일 · pytest']];
...
const narratives = [
  ['외부 구독 연동의 한계 → 카탈로그 기반 전환',
   ...
   '서비스 98종·요금제 196개·카테고리 15개 카탈로그 구축 → 검색·선택만으로 등록. 이 카탈로그가 이후 요금 인상 감지·중복 구독 판정의 전제 조건이 됨.'],
  ...
  ['금액 기준 통일 → 취향이 아니라 데이터가 강제한 요구사항',
   '요금제 196개 중 96개가 외화(USD) 기준이다. 카탈로그의 절반이 환율에 노출돼 있고 해외 서비스는 표시가에 부가세 10%까지 붙어, 주기·통화·세금을 각자 환산하면 총액과 비중이 조용히 어긋난다.',
...
const demoFeatures = [
  ...
  ['02','대시보드','월/연 총 지출·활성 구독 수·다음 결제일·구독 건강 점수를 한 화면에 요약. 7일 내 결제 예정 위젯으로 임박 결제 강조.'],
  ['03','지출 분석','카테고리별 지출 비중 도넛 + 3/6/12/24개월 추이 라인 차트. 다운그레이드·해지·주기 전환 절약 인사이트를 원클릭 적용.'],
  ...
  ['06','고급 인사이트','같은 카테고리 중복 구독 감지, 외화 구독 환율 추적(부가세 10% 포함 KRW 환산), 가족·팀 공유 구독의 1인당 분담액, 요금 인상 이력(plan_price_history) 관리.'],
